...
const TEXTAREAS = [
  {key:"description",    label:"About You",       placeholder:"I am a passionate developer who loves building web apps...",           icon:"🙋" },
  {key:"skills",         label:"Skills",           placeholder:"Python, Django, React, PostgreSQL, Git...",                            icon:"⚡" },
  {key:"experience",     label:"Work Experience",  placeholder:"Intern at TechCorp for 3 months. Built REST APIs using Django...",     icon:"💼" },
  {key:"education",      label:"Education",        placeholder:"B.E Computer Science, Anna University, 2024, CGPA 8.2",               icon:"🎓" },
  {key:"projects",       label:"Projects",         placeholder:"1. HireWise - AI resume analyzer using Django and OpenAI\n2. E-commerce site...", icon:"🚀" },
  {key:"achievements",   label:"Achievements",     placeholder:"Won college hackathon. Published blog on Medium...",                   icon:"🏆" },
  {key:"certifications", label:"Certifications",   placeholder:"Python for Data Science - Coursera...",                               icon:"📜" },
];
function FloatInput({ fieldKey,label,placeholder,icon,value,onChange }) {
...
function FloatTextarea({ fieldKey, label, placeholder, icon, value, onChange }) {
  const [focused, setFocused] = useState(false);
  const active = focused || !!value;
  ...
  return (
    <div style={{
      ...
      <span style={{
        ...
        fontSize: 14, opacity: focused ? 1 : 0.5,
      ...
      <textarea
        ...
        style={{
          ...
          fontSize:13,color: "#0f172a",
          fontFamily:"Inter, sans-serif",
          resize:"vertical", transition: "padding 0.18s ease",
...
function Section({ title, children }) {
  return (
    <div style={{ marginBottom: 20 }}>
      <div style={{ fontSize: 11, fontWeight: 700, color: "#2563eb", textTransform: "uppercase", letterSpacing: "0.08em", borderBottom: "1.5px solid #2563eb", paddingBottom: 4, marginBottom: 12 }}>{title}</div>
      {children}
    </div>
  );
}
...
function Tag({ text, color, textColor, small }) {
  return (
    <span style={{ background: color, color: textColor, padding: small ? "2px 8px" : "3px 10px", borderRadius: 20, fontSize: small ? 11 : 12, fontWeight: 500 }}>{text}</span>
  );
}
    ...
    <>
      <style>{`
        ...
        @media (max-width: 700px) {
          ...
          .left-panel    { width: 100% !important; border-right: none !important; border-bottom: 1px solid #f1f5f9; max-height: none !important; }
      ...
      <div className="resume-layout" style={{ minHeight: "100vh", display: "flex", fontFamily: "Inter, sans-serif", background: "#fff" }}>

        {/* ══ LEFT — Form ══ */}
        <div className="left-panel" style={{ width: "44%", borderRight: "1px solid #f1f5f9", padding: "36px 28px", overflowY: "auto", flexShrink: 0 }}>

          <h2 style={{ fontSize: 20, fontWeight: 600, color: "#0f172a", marginBottom: 3 }}>Resume Builder</h2>
          <p style={{ fontSize: 13, color: "#64748b", marginBottom: 20, lineHeight: 1.5 }}>Fill in your details and we'll craft a professional resume</p>
          ...
          <button
            ...
            style={{
              ...
              fontWeight: 600, cursor: phase === "scanning" ? "not-allowed" : "pointer",
        ...
        <div className="right-panel" style={{ flex: 1, padding: "36px 28px", overflowY: "auto", minWidth: 0 }}>
          {phase === "idle" && (
            ...
            <div style={{ animation: "fadeUp 0.45s ease" }}>
              ...
              <div style={{
                ...
                <Section title="Summary">
                  <p style={{ fontSize: 13, color: "#374151", lineHeight: 1.7 }}>{resume.summary}</p>
                </Section>
                  ...
                  <Section title="Experience">
                    {resume.experience.map((e, i) => (
                      <div key={i} style={{ marginBottom: 14 }}>
                        ...
                        <div style={{ fontSize: 12, color: "#64748b", marginBottom: 6 }}>{e.duration}</div>
                  ...
                  {(resume.education || []).map((e, i) => (
                    <div key={i} style={{ marginBottom: 10 }}>
                      <div style={{ display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: 4 }}>
                        ...
                        <span style={{ fontSize: 12, color: "#64748b" }}>{e.year}</span>
                      </div>
                      <div style={{ fontSize: 12, color: "#64748b" }}>{e.institution}{e.cgpa ? ` · ${e.cgpa}` : ""}</div>
                    </div>
                  ))}
                </Section>
                  ...
                  <Section title="Projects">
                    {resume.projects.map((p, i) => (
                      <div key={i} style={{ marginBottom: 14 }}>
                        ...
                        <p style={{ fontSize: 13, color: "#374151", marginBottom: 4 }}>{p.description}</p>
                        <ul style={{ paddingLeft: 18 }}>
                ...
                {resume.achievements?.length > 0 && (
                  <Section title="Achievements">
                    <ul style={{ paddingLeft: 18 }}>
                      {resume.achievements.map((a, i) => <li key={i} style={{ fontSize: 13, color: "#374151", marginBottom: 4 }}>{a}</li>)}
                    </ul>
                  </Section>
                )}
              </div>
